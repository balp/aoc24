...
function findPointOnLine(x1: number, y1: number, x2: number, y2: number, distance: number): { x: number, y: number } {
  ...
  function in_matrix(point: { x: number; y: number }) {
  ...
  for (const frequency in antennas) {
    ...
    for (const first of antennas[frequency]) {
      for (const second of antennas[frequency]) {
        ...
        while (true) {
          c++
          const p1 = findPointOnLine(first[0], first[1], second[0], second[1], -between*c);
          if (!in_matrix(p1)) break;
          plot(input, p1);
        }
        c = 0
        while (true) {
          c++
          const p2 = findPointOnLine(second[0], second[1], first[0], first[1], -between*c);
          if (!in_matrix(p2)) break;
          plot(input, p2);
        }
